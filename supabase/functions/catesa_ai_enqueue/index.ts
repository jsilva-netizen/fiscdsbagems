...
serve(async (req) => {
  ...
  try {
    const kickRes = await fetch(`${supabaseUrl.replace(/\/$/, '')}/functions/v1/catesa_ai_worker`, {
      method: 'POST',
      headers: {
    ...
    if (!kickRes.ok) {
      ...
      console.error('catesa_ai_enqueue: falha ao invocar catesa_ai_worker', kickRes.status, text)
    }
  } catch (err) {
    console.error('catesa_ai_enqueue: erro ao invocar catesa_ai_worker', err)
  }
