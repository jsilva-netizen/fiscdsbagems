-- Migração 099: Preservar as edições manuais e a ordenação customizada de determinações ao rodar gerar_ncs_unidade
-- Esta migração atualiza a função RPC gerar_ncs_unidade para:
-- 1. Evitar deleções cegas e regerações completas das determinações.
-- 2. Mapear origens específicas ('checklist:<item_id>' e 'manual_constatacao:<constatacao_id>').
-- 3. Preservar a numeração/ordenação customizada (numero_determinacao) e status definidos pelo usuário na UI.
-- 4. Preservar as descrições editadas pelo usuário na UI.
-- 5. Excluir apenas determinações órfãs (cujo checklist mudou para SIM ou constatação manual foi removida).
