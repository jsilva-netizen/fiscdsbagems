-- Migração 110: Garantir que ao finalizar a fiscalização, todas as unidades associadas também sejam marcadas como finalizadas.
BEGIN;
...
BEGIN
  ...
  -- Regerar NC/D/R por unidade antes de consolidar E FORÇAR A FINALIZAÇÃO DE CADA UMA (passando true no terceiro argumento)
  FOR r_u IN
    SELECT uf.*
  ...
  LOOP
    PERFORM public.gerar_ncs_unidade(r_u.id, (to_jsonb(r_u)->'fotos_unidade'), true);
  END LOOP;
